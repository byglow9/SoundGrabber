...
| Auditoria de seguranca final do sistema de Submissoes (2026-07-22): caps de campo recalibrados para 300 (titulo/genero/nome/contato) e 700 (descricao), `_MAX_BODY_BYTES` 5120→8192, depois artistas/produtores 3→5 (`_MAX_BODY_BYTES` 8192→11264); caracteres de controle ASCII e overrides Unicode bidi/zero-width bloqueados em todo campo de texto livre; `youtube_url` valida charset do video_id, nao so tamanho | Pedido explicito do usuario para uma verificacao completa antes de abrir a superficie publica de escrita: "todo um sistema de barrar enviar mais de 300 caracteres... SQL injection, injecao de javascript e afins". Pesquisa externa feita (FastAPI/Pydantic OWASP hardening, NoSQL/Redis injection, SSRF/unicode homograph/ReDoS — ver SEC-SUBMIT-08 no SECURITY-CHECKLIST.md para o raio-x completo). Achados: (1) caps antigos (150/350) ja eram mais apertados que o pedido (300/700) — afrouxados a pedido do usuario, com payload de cardinalidade maxima remedido (6676/6661 bytes) e body limit global recalibrado com margem de ~23%; (2) SQL injection nao se aplica (sem banco SQL, storage e Redis via json.dumps/loads); (3) NoSQL/Redis injection ja mitigada pela validacao de submission_id via JOB_ID_PATTERN antes de qualquer lookup; (4) SSRF nao se aplica (nenhuma URL de submissao e fetchada server-side, so renderizada client-side); (5) ReDoS nao se aplica (todos os regex de validacao sao classes de caracteres simples, sem backtracking catastrofico); (6) gap real encontrado e fechado: nenhum campo bloqueava caracteres de controle ASCII nem overrides Unicode bidi (RLO/LRO etc., ataque estilo "Trojan Source") que um submissor poderia usar pra enganar visualmente o operador antes de aprovar; (7) segundo gap fechado: `_normalize_youtube_url` validava so o tamanho do video_id (11 chars), nao o charset, permitindo um valor percent-encoded com markup passar pelo check antigo. Superficie de abuso permanece contida — todos os limites seguem ordens de grandeza abaixo de qualquer payload de ataque relevante. Flexibilizacao/hardening registrados conforme Security Gate; 9 testes novos cobrem os caps, caracteres de controle, bidi override, youtube_url e XSS/SQLi-style payloads, mais um teste que converteu a inspecao manual de innerHTML em `yonkou.js` para automatizada. |
| pip-audit no predeploy-check ignora tambem PYSEC-2026-3622 / CVE-2026-55404 (2026-09-10) | yt-dlp 2026.3.17 — injecao de comando via a opcao `--write-link` (gera arquivos de atalho .desktop/.url/.webloc com dado de video nao sanitizado; corrigido em 2026.7.4). Verificado via `grep` em `pipeline.py`/`api/*.py`: o projeto nunca passa `--write-link`/`--write-url-link`/`--write-webloc-link`/`--write-desktop-link` ao yt-dlp, logo a superficie vulneravel nao e exercida. Ignorado para destravar o deploy emergencial da correcao de persistencia do Redis (perda de dados em 2026-09-10 por queda de energia com Redis sem `--save`/`--appendonly`). Mesma categoria/precedente do row acima (bump definitivo do yt-dlp pendente de validacao); ao atualizar, remover tambem este `--ignore-vuln`. Flexibilizacao registrada conforme Security Gate. |
