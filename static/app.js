...
function renderFeatured(data) {
  ...
  if (links.some(item => item && item.label && item.url)) {
    ...
    links.forEach(item => {
      ...
      // LINK_ICONS é markup SVG estático e confiável (constantes do código, sem
      // dado do usuário). Parseia com DOMParser e importa o nó — renderiza o ícone
      // da plataforma sem manipular HTML como string (contrato anti-XSS do app.js).
      const iconDoc = new DOMParser().parseFromString(LINK_ICONS[iconKey], 'image/svg+xml');
